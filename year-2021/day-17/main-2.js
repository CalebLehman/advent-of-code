...
var countValidShots = function (xRange, yRange) {
    var count = 0;
    for (var ySpeed = yRange.lo; ySpeed <= xRange.hi; ySpeed += 1) {
        var minStep = Math.ceil((2 * ySpeed + 1 + Math.sqrt((2 * ySpeed + 1) * (2 * ySpeed + 1) - 8 * yRange.hi)) / 2);
        var maxStep = Math.floor((2 * ySpeed + 1 + Math.sqrt((2 * ySpeed + 1) * (2 * ySpeed + 1) - 8 * yRange.lo)) / 2);
        for (var xSpeed = Math.ceil((-1 + Math.sqrt(1 + 8 * xRange.lo)) / 2); xSpeed <= xRange.hi; xSpeed += 1) {
            for (var step = minStep; step <= maxStep; step += 1) {
                var x = computeHorizontalPosition(xSpeed, step);
                if ((x >= xRange.lo) && (x <= xRange.hi)) {
                    count += 1;
                    console.log("".concat(xSpeed, ",").concat(ySpeed));
                    break;
                }
            }
        }
    ...
    return count;
    for (var v = xRange.hi; v >= yRange.lo; v -= 1) {
        ...
        for (var step = minStep; step <= maxStep; step += 1) {
            ...
            for (var speed = minSpeed; speed <= xRange.hi; speed += 1) {
                ...
                if ((x >= xRange.lo) && (x <= xRange.hi)) {
                    count += 1;
                    console.log("".concat(speed, ",").concat(v));
                }
            }
        }
    }
...
fs.readFile("input.txt", "utf-8", function (err, data) {
    ...
    var shots = countValidShots(xRange, yRange);
    console.log(shots);
});
