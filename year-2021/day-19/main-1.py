...
def main():
    ...
    while len(border_scanners) > 0:
        ...
        for unmatched_scanner in unmatched_scanners:
            matched_scanner = checkAgainstReference(unmatched_scanner, border_scanner, 12)
...
def checkAgainstReference(points, reference, cutoff=12):
    ...
    for turn in turns:
        ...
        if count >= cutoff:
            return [addPoint(point, difference) for point in turned_points]
    return []
